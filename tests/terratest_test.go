...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-eth-qos-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Ethernet QoS Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/vnic/EthQosPolicies/%s", moid), expectedJSONTemplate, vars)
}
